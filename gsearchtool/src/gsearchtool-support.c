...
/* START OF GENERIC GDE2-SEARCH-TOOL FUNCTIONS */
...
is_path_hidden (const gchar * path)
{
	...
	if (sub_str != NULL) {
		gchar * gde2_desktop_str;

		gde2_desktop_str = g_strconcat (G_DIR_SEPARATOR_S, ".gde2-desktop", G_DIR_SEPARATOR_S, NULL);

		/* exclude the .gde2-desktop folder */
		if (strncmp (sub_str, gde2_desktop_str, strlen (gde2_desktop_str)) == 0) {
			sub_str++;
...
is_quick_search_excluded_path (const gchar * path)
{
	...
	settings = g_settings_new ("org.gde2.search-tool");
...
gsearchtool_pixmap_file (const gchar * partial_path)
{
	...
	path = g_build_filename(DATADIR "/pixmaps/gde2-search-tool", partial_path, NULL);
