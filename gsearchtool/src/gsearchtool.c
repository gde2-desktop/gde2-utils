...
#define GDE2_SEARCH_TOOL_DEFAULT_ICON_SIZE 16
#define GDE2_SEARCH_TOOL_STOCK "panel-searchtool"
#define GDE2_SEARCH_TOOL_REFRESH_DURATION  50000
...
start_animation (GSearchWindow * gsearch, gboolean first_pass)
{
	if (first_pass == TRUE) {
		...
		if (g_settings_get_boolean (gsearch->gde2_desktop_interface_settings, "enable-animations")) {
                      ...
                      gboolean first_pass)
{
	...
	if ((gtk_widget_get_visible (gsearch->available_options_vbox) == FALSE) ||
		...
		if (gsearch->command_details->is_command_first_pass == TRUE) {
			...
			locate = g_find_program_in_path ("locate");
			disable_quick_search = g_settings_get_boolean (gsearch->gde2_search_tool_settings, "disable-quick-search");
			gsearch->command_details->is_command_second_pass_enabled = !g_settings_get_boolean (gsearch->gde2_search_tool_settings, "disable-quick-search-second-scan");
                                  ...
                                  gboolean flag)
{
	...
	select_settings = g_settings_new ("org.gde2.search-tool.select");
				 ...
				 gpointer data)
{
	...
	if (condition & G_IO_IN) {
		...
		do {
			...
			if (duration > GDE2_SEARCH_TOOL_REFRESH_DURATION) {
				if (gtk_events_pending ()) {
					intermediate_file_count_update (gsearch);
					while (gtk_events_pending ()) {
						if (gsearch->command_details->command_status == MAKE_IT_QUIT) {
...
register_gsearchtool_icon (GtkIconFactory * factory)
{
	...
	gtk_icon_source_set_icon_name (source, GDE2_SEARCH_TOOL_ICON);

	icon_set = gtk_icon_set_new ();
	gtk_icon_set_add_source (icon_set, source);

	gtk_icon_factory_add (factory, GDE2_SEARCH_TOOL_STOCK, icon_set);
...
gsearchtool_init_stock_icons (void)
{
	...
	gsearchtool_icon_size = gtk_icon_size_register ("panel-menu",
							GDE2_SEARCH_TOOL_DEFAULT_ICON_SIZE,
							GDE2_SEARCH_TOOL_DEFAULT_ICON_SIZE);
...
{
	if (g_settings_get_boolean (gsearch->gde2_search_tool_settings, "show-additional-options")) {
		if (gtk_widget_get_visible (gsearch->available_options_vbox) == FALSE) {
			gtk_expander_set_expanded (GTK_EXPANDER (gsearch->show_more_options_expander), TRUE);
			gtk_widget_show (gsearch->available_options_vbox);
		}
	...
	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "contains-the-text")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_CONTAINS_THE_TEXT, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "date-modified-less-than")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_DATE_MODIFIED_BEFORE, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "date-modified-more-than")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_DATE_MODIFIED_AFTER, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "size-at-least")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_SIZE_IS_MORE_THAN, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "size-at-most")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_SIZE_IS_LESS_THAN, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "file-is-empty")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_FILE_IS_EMPTY, NULL, FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "owned-by-user")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_OWNED_BY_USER, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "owned-by-group")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_OWNED_BY_GROUP, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "owner-is-unrecognized")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_OWNER_IS_UNRECOGNIZED, NULL, FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "name-does-not-contain")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_FILE_IS_NOT_NAMED, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "name-matches-regular-expression")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_FILE_MATCHES_REGULAR_EXPRESSION, "", FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "show-hidden-files-and-folders")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_SHOW_HIDDEN_FILES_AND_FOLDERS, NULL, FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "follow-symbolic-links")) {
		add_constraint (gsearch, SEARCH_CONSTRAINT_FOLLOW_SYMBOLIC_LINKS, NULL, FALSE);
	}

	if (g_settings_get_boolean (gsearch->gde2_search_tool_select_settings, "exclude-other-filesystems")) {
...
gsearch_app_create (GSearchWindow * gsearch)
{
	...
	gsearch->gde2_search_tool_settings = g_settings_new ("org.gde2.search-tool");
	gsearch->gde2_search_tool_select_settings = g_settings_new ("org.gde2.search-tool.select");
	gsearch->gde2_desktop_interface_settings = g_settings_new ("org.gde2.interface");
	...
	gsearch->is_window_maximized = g_settings_get_boolean (gsearch->gde2_search_tool_settings, "default-window-maximized");
	...
	locale_string = g_settings_get_string (gsearch->gde2_search_tool_settings, "look-in-folder");
      ...
      char * argv[])
{
	...
	context = g_option_context_new (N_("- the GDE2 Search Tool"));
	...
	gtk_window_set_default_icon_name (GDE2_SEARCH_TOOL_ICON);
	...
	gtk_window_set_wmclass (GTK_WINDOW (gsearch->window), "gde2-search-tool", "gde2-search-tool");
