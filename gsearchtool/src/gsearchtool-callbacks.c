...
store_window_state_and_geometry (GSearchWindow *gsearch)
{
	...
	g_settings_set_int (gsearch->gde2_search_tool_settings,
	                    "default-window-width",
	                    gsearch->window_width);
	g_settings_set_int (gsearch->gde2_search_tool_settings,
	                    "default-window-height",
		            gsearch->window_height);
	g_settings_set_boolean (gsearch->gde2_search_tool_settings,
               ...
               gpointer data)
{
	...
	gtk_show_uri (gtk_widget_get_screen (widget), "help:gde2-search-tool",  
                           ...
                           gpointer data)
{
	...
	if (value != NULL) {
		g_settings_set_string (gsearch->gde2_search_tool_settings, "look-in-folder", value);
	}
	g_free (value);
}
                 ...
                 gpointer client_data)
{
	...
	set_clone_command (gsearch, &argc, &argv, "gde2-search-tool", FALSE);
                         ...
                         gpointer data)
{
	...
	if (response == GTK_RESPONSE_OK) {
		g_settings_set_boolean (gsearch->gde2_search_tool_settings, "disable-quick-search", TRUE);
	}
}
                    ...
                    gpointer user_data)
{
	...
	if (g_slist_length (order) == NUM_VISIBLE_COLUMNS) {
		g_settings_set_value (gsearch->gde2_search_tool_settings, "columns-order", g_variant_new ("ai", &array_builder));
	}
	g_slist_free (order);
}
