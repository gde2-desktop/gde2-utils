...
/* logview settings */
#define LOGVIEW_SCHEMA "org.gde2.system-log"
...
#define GDE2_MONOSPACE_FONT_NAME "monospace-font-name"
#define GDE2_MENUS_HAVE_TEAROFF  "menus-have-tearoff"
...
logview_prefs_init (LogviewPrefs *self)
{
  ...
  priv->interface_prefs = g_settings_new ("org.gde2.interface");

  g_signal_connect (priv->interface_prefs, "changed::" GDE2_MONOSPACE_FONT_NAME,
                    G_CALLBACK (monospace_font_changed_cb), self);
  g_signal_connect (priv->interface_prefs, "changed::" GDE2_MENUS_HAVE_TEAROFF,
...
logview_prefs_get_monospace_font_name (LogviewPrefs *prefs)
{
  ...
  return (g_settings_get_string (prefs->priv->interface_prefs, GDE2_MONOSPACE_FONT_NAME));
}
...
logview_prefs_get_have_tearoff (LogviewPrefs *prefs)
{
  g_assert (LOGVIEW_IS_PREFS (prefs));
	...
	return (g_settings_get_boolean (prefs->priv->interface_prefs, GDE2_MENUS_HAVE_TEAROFF));
}
